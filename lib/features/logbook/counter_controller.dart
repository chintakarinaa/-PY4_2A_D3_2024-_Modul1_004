...
class CounterController {
  ...
  Future<void> loadData(String username) async {
    ...
    final historyData = prefs.getString('history_$username');
    if (historyData != null) {
  ...
  Future<void> increment(String username) async {
    _counter += _step;
    await _addHistory("User $username menambah +$_step", username);
  }

  Future<void> decrement(String username) async {
    if (_counter - _step >= 0) {
      _counter -= _step;
      await _addHistory("User $username mengurangi -$_step", username);
    }
  }

  Future<void> reset(String username) async {
    _counter = 0;
    await _addHistory("User $username mereset counter", username);
  }
}
